    ...
    m_bDragBottom(false)
{
    ...
    QObject::connect(qApp, &QGuiApplication::applicationStateChanged, this,
        &FrameLessDemo::on_applicationStateChanged);
...
void FrameLessDemo::on_restoreButton_clicked() {
    ui.restoreButton->setVisible(false);
...
void FrameLessDemo::on_maximizeButton_clicked() {
...
void FrameLessDemo::setContent(QWidget* w) {
    ui.windowContent->layout()->addWidget(w);
}

void FrameLessDemo::setWindowTitle(const QString& text) {
    ui.titleText->setText(text);
}

void FrameLessDemo::setWindowIcon(const QIcon& ico) {
    ui.icon->setPixmap(ico.pixmap(16, 16));
}

void FrameLessDemo::styleWindow(bool bActive, bool bNoState) {
    if (bActive) {
        if (bNoState) {
            layout()->setMargin(15);
...
void FrameLessDemo::on_applicationStateChanged(Qt::ApplicationState state) {
    if (windowState().testFlag(Qt::WindowNoState)) {
...
void FrameLessDemo::on_minimizeButton_clicked() {
    setWindowState(Qt::WindowMinimized);
}

void FrameLessDemo::on_closeButton_clicked() { close(); }

void FrameLessDemo::on_windowTitlebar_doubleClicked() {
    if (windowState().testFlag(Qt::WindowNoState)) {
...
void FrameLessDemo::mouseDoubleClickEvent(QMouseEvent* event) {
    Q_UNUSED(event);
}

void FrameLessDemo::checkBorderDragging(QMouseEvent* event) {
    if (isMaximized()) {
        return;
    }
...
bool FrameLessDemo::leftBorderHit(const QPoint& pos) {
...
bool FrameLessDemo::rightBorderHit(const QPoint& pos) {
    const QRect& rect = this->geometry();
...
bool FrameLessDemo::topBorderHit(const QPoint& pos) {
    const QRect& rect = this->geometry();
...
bool FrameLessDemo::bottomBorderHit(const QPoint& pos) {
    const QRect& rect = this->geometry();
...
void FrameLessDemo::mousePressEvent(QMouseEvent* event) {
    if (isMaximized()) {
        return;
    }
...
void FrameLessDemo::mouseReleaseEvent(QMouseEvent* event) {
    Q_UNUSED(event);
    if (isMaximized()) {
        return;
    }
...
bool FrameLessDemo::eventFilter(QObject* obj, QEvent* event) {
    if (isMaximized()) {
